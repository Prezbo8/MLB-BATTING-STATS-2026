...
def get_todays_games() -> list[dict]:
    """Return list of {gamePk, awayAbbr, homeAbbr, gameTime} for today."""
...
def get_lineup(game_pk: int) -> dict | None:
    ...
    Returns {away: [...], home: [...], awayPitcher: {...}, homePitcher: {...}}
    ...
    # MLB API returns empty lists if lineup not yet posted
    ...
    def parse_batters(batters):
        ...
        for i, p in enumerate(batters):
            ...
            bat_side = p.get("batSide", {}).get("code", "")  # R / L / S
            order.append({
                "order":    i + 1,
                "name":     p.get("fullName", ""),
                "position": pos_name,
    ...
    def parse_pitcher(pitcher_list):
        ...
        p = pitcher_list[0]
        name      = p.get("fullName", "")
        pitch_hand = p.get("pitchHand", {}).get("code", "")  # R / L
...
def get_already_confirmed() -> set[str]:
    """
    Return set of team abbrs that are already marked Confirmed in Supabase for today.
...
def upsert_lineup(team_abbr: str, side: str, game: dict, lineup_data: dict, status: str):
...
def main():
    ...
    # Remove stale data from previous days
    supabase.table("projected_lineups").delete().lt("game_date", TODAY).execute()
    logging.info("🗑️  Cleared old game dates")
    ...
    newly_confirmed: list[str] = []  # team abbrs confirmed this run
    ...
    for game in games:
        ...
        if lineup:
            # Lineup is confirmed — upsert both teams
            for abbr, side, name in [
                ...
                # Only notify if this team wasn't already confirmed in a previous run
                if abbr not in already_confirmed:
                    newly_confirmed.append((abbr, name, side, game["gameTime"]))
        else:
            # Not confirmed yet — leave any existing RotoGrinders projected
            # row untouched. Only log that we're still waiting.
            for abbr, side in [(away_abbr, "Away"), (home_abbr, "Home")]:
                logging.info(f"    ⏳ {abbr} ({side}) not yet confirmed — leaving existing projected row")

    # Send one Telegram message per newly confirmed team
