"""
FanGraphs Team Splits — HISTORICAL MONTHLY DUMP
===============================================
One-off / re-runnable backfill that captures TEAM batting stats one calendar
month at a time (March→October) and stores them in a SEPARATE Supabase table
(`fangraphs_splits_history`) so the live `fangraphs_splits` table is untouched.

Difference from the daily scraper (scrape_team_splits.py):
...
# ── Supabase ──────────────────────────────────────────────────────────────────
SUPABASE_TABLE = "fangraphs_splits_history"

# ── Settings ──────────────────────────────────────────────────────────────────
OUTPUT_DIR          = "fangraphs_splits_history"
COMBINED_CSV        = os.path.join(OUTPUT_DIR, "_ALL_SPLITS_HISTORY.csv")
...
def main():
    ...
    print(f"\n🚀 FanGraphs TEAM Splits — Historical Monthly Dump — {today_str}")
    ...
    send_telegram(
        f"⚾ <b>Team Splits History Dump Started</b>\n"
        f"📅 {today_str}\n"
    ...
    try:
        ...
        print(f"\n💥 Crash:\n{err}")
        send_telegram(
            f"💥 <b>Team Splits History Dump CRASHED</b>\n📅 {today_str}\n❌ {str(e)[:200]}"
        )
        raise
    ...
    if failed:
        ...
        send_telegram(
            f"❌ <b>Team Splits History Dump GAVE UP</b>\n"
    ...
    send_telegram(
        f"✅ <b>Team Splits History Dump Done</b>\n"
        f"📅 {today_str}\n🗓️ {', '.join(month_keys)}\n"
